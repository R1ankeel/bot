...
def get_mentioned_usernames(text: str, exclude: set[str] | None = None, limit: int = 3) -> list[str]:
    ...
    for user in get_known_usernames():
        ...
        if len(user_lower) <= 4:
            ...
            # Для длинных ников требуем более точное совпадение
            # (слово целиком или с границами)
            pattern = r'(?<!\w)' + re.escape(user_lower) + r'(?!\w)'
            found = bool(re.search(pattern, t))
...
def is_known_user(username: str) -> bool:
    """Проверяет, есть ли у нас хоть какие-то данные о пользователе."""
    if not username:
        return False

    rep = get_reputation(username)
    # Если уровень не нейтральный или есть прогресс — это реальный пользователь
    if rep["level"] != 6 or rep.get("progress", 0) > 0:
        return True
    if get_facts(username, limit=1):
        return True
    if get_traits(username, limit=1):
        return True
    return False
...
def extract_target_username(text: str, asker: str = "") -> str:
    ...
    # КЛЮЧЕВАЯ ПРОВЕРКА: если такого человека в базе нет — это не ник
    if not is_known_user(candidate):
        return ""
...
def build_hate_style_block(username: str) -> str:
    ...
    styles = [
        "абсурдная метафора: сравни человека с чем-то нелепым, гнилым или невозможным",
        "семейная прожарка: обыграй его родню, генетику, 'как в семье петухов могла появиться такая курочка'",
        "чёрный юмор + слово: 'тебе, чтоб лишиться девственности, сначала придется откопать труп на кладбище'",
        "каламбур / wordplay: играй со словами, рифмуй, используй неожиданные созвучия",
        "интеллектуальный укол с абсурдной логикой: 'ты такой злой, потому что у тебя 2 отца?'",
        "бытовое унижение через абсурд: сравни с протухшим продуктом, сломанной техникой, мусорным баком",
        "театральное отвращение + сарказм: 'ах, какой же ты... уникальный экземпляр. Жаль, что не в музее уродов'",
        "ядовитый 'нет — пидора ответ' стиль: коротко, но с подколом",
        "сравнение с животным/насекомым/паразитом в неожиданном ракурсе",
        "мемный/интернет-стиль прожарки (но без прямых мемов)",
    ]

    recent = state.get_recent_hate_roast_styles(username)
    available = [s for s in styles if s not in recent] or styles
    chosen = random.choice(available)
    state.record_hate_roast_style(username, chosen)

    return (
        "── Стиль ненависти (режим прожарки) ──\n"
        f"Используй этот стиль: **{chosen}**.\n\n"
        "ПРАВИЛА КРЕАТИВА:\n"
        "• Каждый ответ должен быть **разным** по структуре и образам.\n"
        "• Используй абсурд, каламбуры, семейные оскорбления, чёрный юмор.\n"
        "• Примеры желаемого стиля: 'Нет — пидора ответ', 'Ты такой злой, потому что у тебя 2 отца?', "
        "'Как в семье петухов могла появиться такая курочка, как ты?', "
        "'Тебе, чтоб лишиться девственности, сначала придется откопать труп на кладбище.'\n"
        "• Запрещено повторять одни и те же образы/оскорбления даже через несколько сообщений.\n"
        "• Можно быть жёстче обычного — это уровень ненависти.\n"
        "────────────────────────"
    )


def build_hate_roast_creativity_block(username: str) -> str:
    rep = get_reputation(username)
    if rep["level"] < 8:
        return ""

    return (
        "── РЕЖИМ ЖЁСТКОЙ ПРОЖАРКИ ──\n"
        "Ты сейчас в состоянии максимальной ненависти. Твоя задача — **троллить изобретательно**, а не просто материться.\n"
        "Лучшие ответы на этом уровне:\n"
        "• содержат неожиданную метафору или абсурдное сравнение\n"
        "• используют каламбур или игру слов\n"
        "• бьют по семейной/генетической теме\n"
        "• имеют структуру 'Нет — пидора ответ' или похожий язвительный приём\n"
        "• звучат остроумно, даже если очень токсично\n\n"
        "Плохой ответ: 'отвали, урод'\n"
        "Хороший ответ: 'С таким лицом ты даже в морге будешь самым живым'\n"
        "────────────────────────"
    )
...
def build_hate_memory_guard_block(username: str, query: str = "") -> str:
    ...
    return (
        ...
        "Не повторяй одни и те же типы оскорблений даже если факты разные.\n"
        "────────────────────────"
    )
...
def get_system_prompt(
    ...
    if username:
        ...
        if rep["level"] >= 8:
            hate_roast_block = build_hate_roast_creativity_block(username)
            if hate_roast_block:
                parts.append(hate_roast_block)
